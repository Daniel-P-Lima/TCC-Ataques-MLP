...
//static const char* BIN_TREE_PATH = "/spiffs/iot23-%d.bin";

static const uint8_t FEATURE_COUNT = 24;
// static const size_t ARENA_SIZE = 80000;
// Node* arena;
// uint16_t node_count;
Instance instance;
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    switch((esp_mqtt_event_id_t)event_id)
    {
        ...
        case MQTT_EVENT_DATA:
        {
            ...
            memcpy((char*)(&instance), event->data, sizeof(Instance));

            printf("Duration: %.3f\n", instance.duration);
            printf("Orig Bytes: %ld\n", instance.orig_bytes);

            char _DstBuf[3];
            esp_mqtt_client_publish(client, "/esp32/pub", itoa(fit(), _DstBuf, 10), 0, 0, 0);
            break;
        }
...
//     arena = (Node*)(std:->malloc(ARENA_SIZE));
...
void app_main(void)
{
    ...
    //arena = (Node*)(malloc(ARENA_SIZE));
    ESP_LOGI(TAG, "[APP] Free memory after arena alloc: %" PRIu32 " bytes", esp_get_free_heap_size());
